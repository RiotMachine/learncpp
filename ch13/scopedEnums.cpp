...
namespace NFL
{
    ...
    void printResult(Team opponent, WltResult result)
    {
        std::cout << "Verses the " << opponent << '\t';
        if (result == WltResult::win)
            std::cout << "win!\n";
        else if (result == WltResult::loss)
            std::cout << "loss :(\n";
        else
            std::cout << "tie, bro\n";
    }
...
int main()
{
    using Season = std::map<NFL::Team, WltResult>;

    Season chiefs;
    chiefs.insert({ NFL::texans, WltResult::loss });
    chiefs.insert({ NFL::eagles, WltResult::loss });
    chiefs.insert({ NFL::colts, WltResult::win });
